...
public class SentryGunController : BaseEnemyController
{
    /// <summary>
    ...
    {
        LookAround();
        SGAnimator.SetBool("Alert", false);
    }
    ...
    public void MeleeHitter(Vector3 pos)
    {
        if (player != null)
        {
            ...
            if (bullettime <= 0)
            {
                ...
                bullets.transform.rotation = Quaternion.Euler(new Vector3(90, 0, 0));
                sp = bullets.AddComponent<SpriteRenderer>();
                sp.sprite = bulletsp;
                bullets.AddComponent<BulletsController>();
                bullets.GetComponent<BulletsController>().damage = Damage_amount;
                col = bullets.AddComponent<BoxCollider>();
                col.size = new Vector3(0.1f, 0.1f, 0.1f);
                col.transform.position = this.gameObject.transform.position;
                col.isTrigger = true;
                bullets.GetComponent<BulletsController>().playerpos = pos;
                bullettime = 2f;
				Casepop ();
            }
        }
	...
	private void Casepop()
	{
		string[] Mess ={string.Format(CasingMess_ONLY)};
